...
修复点：
- 提供 main() 入口，满足 app.py 的 /api/jobs/calc_scores 动态调用。
- 评分 SQL 对齐当前 DWD/DWS 建表口径，并尽量兼容两种代码格式：
  * BaoStock 格式：sh.600000 / sz.000001
  * Tushare 格式：600000.SH / 000001.SZ

说明：本脚本不会写入行业中性字段（roe_rank/pe_rank/...），行业 Rank 标准化仍由你后续行业中性 SQL 来补。
"""
...
def run_calc_scores(trade_date: Optional[dt.date] = None) -> None:
    ...
    try:
        ...
        # growth（缺 revenue_yoy/revenue_qoq 时，优先用 yoy_pni / yoy_ni 兜底）
        g_yoy = _pick(cols_growth, "revenue_yoy", "yoy_pni", "yoy_ni", "YOYPNI", "YOYNI")
        g_qoq = _pick(cols_growth, "revenue_qoq")  # 通常没有，保留占位
        ...
        SELECT
            ...
            (
                {WEIGHTS['profit']}    * ISNULL(
                    CASE
                        WHEN pf.roe_pct IS NULL THEN 0
                        WHEN pf.roe_pct >= 20 THEN 100
                        WHEN pf.roe_pct >= 15 THEN 80
                        WHEN pf.roe_pct >= 10 THEN 60
                        WHEN pf.roe_pct >= 5  THEN 40
              ...
              + {WEIGHTS['growth']}    * ISNULL(
                    CASE
                        WHEN gr.yoy_pct IS NULL THEN 0
                        WHEN gr.yoy_pct >= 30 THEN 100
                        WHEN gr.yoy_pct >= 15 THEN 70
                        WHEN gr.yoy_pct >= 5  THEN 50
                        ELSE 30
              ...
              + {WEIGHTS['cashflow']}  * ISNULL(
                    CASE
                        WHEN cf.ocf_np_raw IS NULL THEN 0
                        WHEN cf.ocf_np_raw >= 1 THEN 100
                        WHEN cf.ocf_np_raw >= 0.7 THEN 70
                        ELSE 40
                    END, 0
                )
              + {WEIGHTS['safety']}    * ISNULL(
                    CASE
                        WHEN bl.dar_pct IS NULL THEN 0
                        WHEN bl.dar_pct <= 40 THEN 100
                        WHEN bl.dar_pct <= 60 THEN 70
                        ELSE 40
                    END, 0
                )
              + {WEIGHTS['valuation']} * ISNULL(
                    CASE
                        WHEN k.pe_ttm IS NULL OR k.pb_mrq IS NULL THEN 0
                        WHEN k.pe_ttm < 10 AND k.pb_mrq < 1 THEN 100
                        WHEN k.pe_ttm < 20 THEN 70
                        ELSE 40
                    END, 0
                )
              + {WEIGHTS['operation']} * ISNULL(
                    CASE
                        WHEN op.asset_turn_raw IS NULL THEN 0
                        WHEN op.asset_turn_raw >= 1.2 THEN 100
                        WHEN op.asset_turn_raw >= 0.8 THEN 70
                        ELSE 40
                    END, 0
                )
              + {WEIGHTS['dividend']}  * ISNULL(
                    CASE
                        WHEN dv.dividend_yield_ttm IS NULL THEN 0
                        WHEN dv.dividend_yield_ttm >= 6 THEN 100
                        WHEN dv.dividend_yield_ttm >= 4 THEN 70
                        ELSE 40
                    END, 0
